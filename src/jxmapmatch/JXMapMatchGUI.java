 ...
 *
 * This class represents the GUI of JXMapMatcher
 * implements all functions demanded by JXMapMatchInterface
 */
...
public class JXMapMatchGUI extends JFrame implements JXMapMatchGUIInterface {
	...
	private JCheckBox jCheckBoxNRouteNormalizeMatchedGPSTimeStamp;
	...
	public JXMapMatchGUI(StatusUpdate statusUpdate) {
		...
		// change toggle button's select color. must be done before creating an instance
	...
	private void initMapMatchingPanel(Container container) {
		...
		/*
		// create JComboBox
		jComboBoxMapMatching = new JComboBox<String>(mapMatchingAlgorithms);
	...
	private void initNRoutePanel(Container container) {
		...
		matchGPStoNRouteAlgorithmMatchButtonCaptions.put(MATCH_GPS_TO_N_ROUTE_RUNNING, "Pause");
		matchGPStoNRouteAlgorithmMatchButtonCaptions.put(MATCH_GPS_TO_N_ROUTE_PAUSED, "Resume");
		...
		SpinnerModel spinnerNumberModelMaxContainerSize = new SpinnerNumberModel(DEFAULT_N_ROUTE_SIZE,		// initial value
		...
		// create label
		jLabelNRouteTreshold = new JLabel("Treshold (" + MIN_INTERSECTION_REACHED_THRESHOLD + " - " + MAX_INTERSECTION_REACHED_THRESHOLD  + ")");
		jLabelNRouteTreshold.setAlignmentX(CENTER_ALIGNMENT);
		// create JSpinner and its eligible model (number model)
		SpinnerModel spinnerNumberModelTreshold = new SpinnerNumberModel(DEFAULT_INTERSECTION_REACHED_THRESHOLD, // init
		...
		// create check box
		jCheckBoxNRouteNormalizeMatchedGPSTimeStamp = new JCheckBox("Normalize");
		jCheckBoxNRouteNormalizeMatchedGPSTimeStamp.setAlignmentX(CENTER_ALIGNMENT);
		
		// create horizontal boxes
		Box horBox1 = Box.createHorizontalBox(),
			horBox2 = Box.createHorizontalBox(),
			horBox3 = Box.createHorizontalBox();
		horBox1.add(jButtonNRouteRoute);
		horBox1.add(jButtonNRouteMatch);
		horBox2.add(jButtonNRouteOpen);
		horBox2.add(jButtonNRouteExport);
		horBox3.add(jButtonNRouteSave);
		horBox3.add(jButtonNRouteReset);
		...
		jPanelNRoute.add(jCheckBoxNRouteNormalizeMatchedGPSTimeStamp);
	...
	private void initStatusBar(Container container) {
		...
		jPanelStatusBar.add(Box.createRigidArea(new Dimension(0,20)), BorderLayout.CENTER); //keeps status bar's height constant
	...
	/**
	 * set custom tile factory for map representation with custom
	 * values for zoom, size and orientation
	 */
	@Override
	public void setTileFactory(String baseURL, String xParam,
			   				   String yParam, String zParam,
			   				   int minZoom,	int maxZoom, 
			   				   final int totalZoom, int tileSize,
			   				   boolean defaultXOrientation,
			   				   boolean defaultYOrientation) {
		
		// initialize custom tile factory
	    TileFactoryInfo tileFactoryInfo = new TileFactoryInfo(minZoom, // minimum Zoom-Level
...
//	        		return "C:\\priv\\uni\\MA\\hintergrund.png";
	...
	public void setTileFactory(String url, String xParam, String yParam, String zParam) {
		...
		setTileFactory(url, xParam, yParam, zParam, MIN_ZOOM_DEFAULT, MAX_ZOOM_DEFAULT, TOTAL_ZOOM_DEFAULT, 
					   TILE_SIZE_DEFAULT, X_ORIENTATION_DEFAULT, Y_ORIENTATION_DEFAULT);
	}
	...
	public void setEventListener(EventListener eventListener) {
		...
		jToggleButtonSelectRoute.addActionListener((ActionListener) eventListener);
		jToggleButtonNRouteAlgorithm.addActionListener((ActionListener) eventListener);
		...
		jxMapKit.getMainMap().addMouseMotionListener((MouseMotionListener) eventListener);
	...
	///////////////////////////////////////////// GUI ENABLE/DISABLE /////////////////////////////////////////////////////////////////////////
	/**
	 * enables/disables GUI in which included panels separately gets regarded according to their state
	 */
	@Override
	public void enableGUI(boolean enable) {
		...
		// set hour glass mouse cursor if GUI gets disabled, otherwise restore normal cursor
		setCursor( enable ? null : Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR) );
	}
	
	/**
	 * enables/disables drawing components according to their permission to be enabled
	 * @param enable
	 */
	...
	/**
	 * enables/disables manual route selecting panel according to their permission to be enabled
	 * @param enable
	 */
	...
	private void enableNRouteAlgorithmPanel(boolean enable) {
		...
		// consider toggle button, spinner and buttons separately considering n route algorithm state
		...
		jCheckBoxNRouteNormalizeMatchedGPSTimeStamp.setEnabled(enableExport);
		jButtonNRouteReset.setEnabled(enableReset);
		
		// get caption of match button according to current match GPS/n route algorithm states
	...
	private void enableNRouteAlgorithmPanel(boolean enable) {
		// check conditions for activating
		boolean enableToogleButton = (enable && isGUIEnabled && isNRouteAlgorithmAvailable);
		boolean enableComponents = enableToogleButton && isNRouteAlgorithmModeSet;

		// enable components if N route algorithm mode is activated
		Tools.enableComponentsInsideContainer(jPanelNRoute, enableComponents);
		
		// consider toggle button seperately cause n route algorithm mode must not be set
		jToggleButtonNRouteAlgorithm.setEnabled(enableToogleButton);
	}
	*/
	
	/**
	 * prepares the rest of the GUI for a map matching modus by disabling not needed components
	 * @param enable
	 */
	...
	/**
	 * this method enables basic elements of the GUI like status bar and JXMapKit
	 */
	...
	public void makeSelectRouteModeAvailable(boolean enable) {
		...
		//Tools.enableComponentsInsideContainer(jPanelSelectRoute, (enable && isGUIEnabled));
	}
	...
	@Override
	public void makeNRouteButtonsAvaiable(boolean route, boolean match,
			boolean save, boolean export, boolean reset) {
	...
	public void setSelectRouteMode(boolean set) {
		...
		// disable/enable N route algorithm panel if this mode is activated/deactivated
	...
	public boolean setNRouteAlgorithmState(String state) {
		...
		if (isNRouteAlgorithmModeSet) {
			...
			if ((state == N_ROUTE_RUNNING) || (state == N_ROUTE_PAUSED) || (state == N_ROUTE_RECESSED)) {
	...
	public void setNormalizeNMatchedGPSTimeStamp(boolean enable) {
		jCheckBoxNRouteNormalizeMatchedGPSTimeStamp.setSelected(enable);
	}


	@Override
	public boolean getNormalizeNMatchedGPSTimeStamp() {
		return jCheckBoxNRouteNormalizeMatchedGPSTimeStamp.isSelected();
	}
	...
	public Point getTranslatedMousePos(MouseEvent mouseEvent){
		...
		//return absolute x- and y-position = (rect.x/y + mouse.x/y) * zoom factor 
		return new Point( (int) ((rect.getX() + mouseEvent.getX()) * zoomFactor),
	...
	public void updateStatus(float percent) {
		// show working state via progress bar, make visible and turn off intermediate mode
		if (jProgressBar.isIndeterminate()) jProgressBar.setIndeterminate(false);
	...
	@Override
	public void updateUndefinedStatus(String undefinedMessage, String updateMessage) {
	...
	public void updateUndefinedStatus() {
		if (!jProgressBar.isIndeterminate()) jProgressBar.setIndeterminate(true);
		if (!jProgressBar.isVisible()) jProgressBar.setVisible(true);
	}
	...
	public void setNRouteSize(int nRouteSize) {
		...
		if (nRouteSize > MAX_N_ROUTE_SIZE) nRouteSize = MAX_N_ROUTE_SIZE;
		if (nRouteSize < MIN_N_ROUTE_SIZE) nRouteSize = MIN_N_ROUTE_SIZE;
	...
	public void setNRouteThreshold(double threshold) {
		...
		threshold = (threshold <= MAX_INTERSECTION_REACHED_THRESHOLD) ? threshold : MAX_INTERSECTION_REACHED_THRESHOLD;
		threshold = (threshold >= MIN_INTERSECTION_REACHED_THRESHOLD) ? threshold : MIN_INTERSECTION_REACHED_THRESHOLD;
