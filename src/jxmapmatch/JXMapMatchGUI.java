...
public class JXMapMatchGUI extends JFrame implements JXMapMatchGUIInterface {
	...
	public JXMapMatchGUI(StatusUpdate statusUpdate) {
		...
		// instance
		Tools.setUIColorForComponent("ToggleButton.select", new Color(250, 250,
				100));
		...
		this.statusUpdate
				.updateStatus("setting Arial (Plain, 11) as label font...");
		Tools.setFontForComponents(this, "javax.swing.JLabel", new Font(
				"Arial", Font.PLAIN, 11));
	...
	private void initNRoutePanel(Container container) {
		...
		jButtonNRouteMatch
				.setActionCommand("change match GPS to N route algorithm state");
		...
		Box horBox1 = Box.createHorizontalBox(), horBox2 = Box
				.createHorizontalBox(), horBox3 = Box.createHorizontalBox();
	...
	private void initSelectRoutePanel(Container container) {
		// create panel
		jPanelSelectRoute = new JPanelBoxLayout(BoxLayout.Y_AXIS,
				container.getWidth(), 5, "Route Select");
	...
	private void enableOverLayPanel(boolean enable) {
		...
		jCheckBoxStreetMap
				.setEnabled((enable && isGUIEnabled && isDrawStreetMapAvailable));
		jCheckBoxGPSTrace
				.setEnabled((enable && isGUIEnabled && isDrawGPSTraceAvailable));
		jCheckBoxNRoute
				.setEnabled((enable && isGUIEnabled && isDrawNRouteAvailable));
		jCheckBoxSelectedRoute
				.setEnabled((enable && isGUIEnabled && isDrawSelectedRouteAvailable));
	}
	...
	private void enableSelectRoutePanel(boolean enable) {
		Tools.enableComponentsInsideContainer(jPanelSelectRoute, (enable
				&& isGUIEnabled && isSelectRouteModeAvailable));
	}
	...
	private void enableNRouteAlgorithmPanel(boolean enable) {
		...
		// algorithm states
		jButtonNRouteRoute.setText(nRouteAlgorithmMatchButtonCaptions
				.get(nRouteAlgorithmState));
		jButtonNRouteMatch.setText(matchGPStoNRouteAlgorithmMatchButtonCaptions
				.get(matchGPStoNRouteAlgorithmState));
	}
	...
	private void enableComponentsForMatchingOperations(boolean enable,
			boolean allowLoadMap, boolean allowLoadGPSTrace) {
	...
	public boolean getDrawSelectedRoute() {
		return jCheckBoxSelectedRoute.isSelected(); // return status of check
													// box
	}
	...
	public void setNRouteAlgorithmMode(boolean set, boolean allowLoadGPSTrace) {
		...
		enableComponentsForMatchingOperations(isNRouteAlgorithmModeSet, !set,
				!set || allowLoadGPSTrace);
	...
	public void updateNRouteSettings() {
		...
		jSliderSelectNRoute
				.setMaximum(((Integer) jSpinnerNRouteSize.getValue())
						.intValue());
		jLabelSelectNRoute.setText("Choose route: "
				+ jSliderSelectNRoute.getValue());
	}
